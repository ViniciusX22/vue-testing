...
`http://localhost:8080`; // specify the start page
...
test("can change news page", async t => {
  await t
    ...
    .expect(getLocation())
    .contains("/top/2")
    .expect(Selector(".news-list-nav>span").withText("2/"))
    .ok("the news nav should show that we changed page");
});
...
test("can see the user page by clicking a user url", async t => {
  await t
    ...
    .expect(getLocation())
    .contains("/user/")
    .expect(Selector(".user-view.view"))
    .ok("the user view have been loaded");
});
...
test("can see the comments page by clicking a comments url", async t => {
  await t
    ...
    .expect(getLocation())
    .contains("/item/")
    .expect(Selector(".item-view-comments-header"))
    .ok("there is a comments header here");
});
